...
import { createImageQRBase64 } from 'src/app/common/utilities/qr';
...
@Component({
  selector: 'app-firma-v3',
  templateUrl: './firma-v3.component.html',
  styleUrls: ['./firma-v3.component.scss'],
})
export class FirmaV3Component {
  title: string = 'FirmaV3Component';
  ...
  initForms() {
    ...
    this.formCertificado = this.fb.group({
      ...
      posicionX: new FormControl(130, Validators.required),
      posicionY: new FormControl(770, Validators.required),
    });
  }
  ...
  async post_firmaPdf() {
    ...
    try {
      ...
      // CREAMOS QR EN BASE-64
      let nombre = 'PRUEBA';
      if (miCertficado && miCertficado.titular && miCertficado.titular.CN) {
        nombre = miCertficado.titular.CN;
      }
      const fechaActual = format(new Date(), 'dd-MM-yyyy HH:mm');
      const fechaHasta = format(
        new Date(
          miCertficado?.validez?.hasta ? miCertficado.validez.hasta : new Date()
        ),
        'dd-MM-yyyy HH:mm'
      );
      const qrImageB64 = await createImageQRBase64(
        'NOMBRE: ' +
          nombre +
          '\nCORREO: ' +
      ...
      this.imageB64 = qrImageB64.split(',')[1];
      // CREAMOS QR EN BASE-64 END
      ...
      const payload: IJacobitusFirmaPdfRequest = {
        ...
        image: this.imageB64,
        point: {
          x: this.formCertificado.value.posicionX,
          y: this.formCertificado.value.posicionY,
        },
      };
